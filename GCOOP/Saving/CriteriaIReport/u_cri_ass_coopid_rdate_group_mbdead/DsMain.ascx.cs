...
namespace Saving.CriteriaIReport.u_cri_ass_coopid_rdate_group_mbdead
{
    public partial class DsMain : DataSourceFormView
    {
        ...
        {
            string sql = @"select assisttype_code,assisttype_code||'-'||assisttype_desc as assisttype_desc,1 as sorter from assucfassisttype where assisttype_group  = '02'
union
...
select '','--เลือกประเภท--',0 from dual order by sorter , assisttype_code";
        ...
        {
            string sql = "select min(assisttype_code) as assisttype_code from assucfassisttype where assisttype_group  = '02'";
        ...
        {
            string sql = "select max(assisttype_code) as assisttype_code from assucfassisttype where assisttype_group  = '02'";
            Sdt dt = WebUtil.QuerySdt(sql);
            if (dt.Next())
            {
