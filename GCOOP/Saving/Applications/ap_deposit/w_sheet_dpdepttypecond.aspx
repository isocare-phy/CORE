    ...
    <script type="text/javascript">
        ...
        function onDwFixItemChange(sender, row, columnName, newValue) {
            ...
            if (columnName == "timedue_flag" || columnName == "havetax_status" || columnName == "clsbfduefee_flag" || columnName == "profit_flag" || columnName == "feenomove_flag" || columnName == "timedue_meth") {
                objDwFix.SetItem(row, columnName, newValue);
                objDwFix.AcceptText();
                Accepttext(row, columnName, newValue);
                postChange();
                return 0;
            }
            ...
            else if (columnName == "due_tdate") {
                objDwFix.SetItem(row, columnName, newValue);
                objDwFix.AcceptText();
                objDwFix.SetItem(row, "due_date", Gcoop.ToEngDate(newValue));
                objDwFix.AcceptText();
                jsPostChgfixDate();
            }
            else {
        ...
        function onDwSavingItemChange(sender, row, columnName, newValue) {
            ...
            else { //เพิ่มให้ accepttext ใน dw ของ ทุก tab เนื่องจากมีฟิวเหมือนกัน ทำให้บันทึกไม่ได้ถ้าไม่ accepttext ทั้งหมด Edit by Mike 047/08/2018
                Accepttext(row, columnName, newValue);
         }
        ...
        function onDwMainItemChange(sender, row, columnName, newValue) {
            ...
            // return 0;
                ...
                sender.AcceptText();
                sender.SetItem(row, "persongrp_code", Gcoop.GetEl("Hdpersong").value);
                sender.AcceptText();
                Gcoop.GetEl("HfDeptType").value = newValue;
                //Gcoop.GetEl("Hdpersong").value = "00";
                postType();
            }
        }
        ...
        function Accepttext( row, columnName, newValue) {
            objDwSaving.SetItem(row, columnName, newValue);
            objDwSaving.AcceptText();
            objDwFix.SetItem(row, columnName, newValue);
            objDwFix.AcceptText();
