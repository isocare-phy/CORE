...
{
    public partial class ws_mbshr_member_detail_main : PageWebSheet, WebSheet
    {
        [JsPostBack]
        ...
        public void InitJsPostBack()
        {
            ...
            dsMthExpense.InitDsMthExpense(this);
        ...
        public void CheckJsPostBack(string eventArg)
        {
            if (eventArg == PostMemberNo)
            {
                ...
                dsMthExpense.RetrieveMthExpense(ls_member_no);
                ...
                try
                {
                    ldtm_birth = dsDetail.DATA[0].BIRTH_DATE;
        ...
        public void SumTotal()
        {
            ...
            for (int i = 0; i < ls_row_count2; i++)
            {
                ...
                decimal ldc_balance = dsEtcpaymonth.DATA[i].ITEM_BALANCE;
                string keepitemtype = dsEtcpaymonth.DATA[i].KEEPITEMTYPE_CODE;
            ...
            dsEtcpaymonth.TContract4.Text = cp_sum_balance.ToString("#,##0.00");

            //คิดผลรวมรายการหักอื่นๆ
            int ls_row_count3 = dsMthExpense.RowCount;
            decimal cp_sum_expense = 0;
            for (int i = 0; i < ls_row_count3; i++)
            {
                decimal ldc_item_payment = dsMthExpense.DATA[i].MTHEXPENSE_AMT;
                cp_sum_expense += ldc_item_payment;


            }
            dsMthExpense.TContract.Text = cp_sum_expense.ToString("#,##0.00");
        }
